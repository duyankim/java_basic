...
public class InputData2 {
	...
	void setSumAve(int start, int lines) {										// 총점 배열을 만드는 메소드다.			
		for (int k03_i = start; k03_i < lines; k03_i++) {
			...
			k03_ave[k03_i] = k03_sum[k03_i] / 3.0;								// 평균 배열의 각 인덱스에 합계 값을 3.0으로 나눈 값을 넣는다.
		}	
	}

	void setSumArr(int page, int start, int limit) {							// 합계 배열을 만드는 메소드다.
		for (int k03_i = start; k03_i < limit; k03_i++) {						// 합계 배열은 한번에 만들지 않고 리스트가 프린트되기 바로 전
			korSum[page] += k03_kor[k03_i];										// 과목의 각 인덱스가 모두 채워지고 난 후에 만든다.
			engSum[page] += k03_eng[k03_i];										// 인자로 받은 시작 인덱스와 마지막 인덱스를 기준으로 반복문을 만든다.
			matSum[page] += k03_mat[k03_i];										// 이 반복문을 돌면 해당 페이지의 합계값이 인덱스 1개에 들어간다.
		}
	}
}
