...
public class P11 {
	...
	public static void main(String[] args) throws IOException {
		...
		File f1 = new File("C:\\Users\\USER\\Desktop\\lesson08\\day_data\\StockDailyPrice.csv");	// 작성할 csv파일을 경로로 설정함
		...
		while ((readtxt = br.readLine()) != null) {													// BufferedReader가 읽어온 라인이 null이 아닐 때까지 반복한다.
			...
			String[] field = readtxt.split("%_%");													// ^%_%^중에서 %_%를 구분자로 field 배열을 만든다.
			
			if (field.length > 2 && field[2].replace("^", "").trim().substring(0, 1).equals("A")) { // 필드의 길이가 2 이상일 때, 종목코드가 A로 시작할 때
				s.append(field[0].replace("^", "").trim());											// 첫 번째 field의 ^를 공백으로 대체하고 공백을 trim해서
																									// StringBuffer s에 추가한다.
				for (int j=1; j<field.length; j++) {												// 두 번째 field부터 마지막 field까지 반복해서  
					s.append("," + field[j].replace("^", "").trim());								// 콤마를 넣은 뒤 ^를 제거한다.
				}
